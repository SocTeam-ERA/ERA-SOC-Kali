...
- **12 machines do not require SMB signing** (including 10.69.0.20, the PXE server). Without it, an attacker who tricks a PC into authenticating can *relay* that authentication to these machines and act as the user. **Ask:** enforce "Microsoft network server: Digitally sign communications (always)" by Group Policy, testing on servers first.
- **4 machines run Windows that no longer receives security updates:** three on Windows 10 2004 (support ended in 2021): 10.69.11.67, 10.69.11.94, 192.168.7.235; and one Windows 11 22H2: 10.69.18.120. Please confirm end-of-support dates against Microsoft's lifecycle page, since the SOC estimates the version from scans. **Ask:** update or replace, or tell us which are exceptions so the alerts can be documented instead of repeated.
...
- **Machines outside the domain:** the scan reads the domain name that Windows machines report (via RDP/NTLM). We will list any that report something other than `era.local` after the next scan (2026-09-23, 12:04), since that data is only now starting to be stored. **Ask:** tell us which are legitimate (guests, lab machines, devices under another group) and which should be joined or removed.

...
- El cambio usa un archivo de estado nuevo (`data/port_state_v2.json`) en vez de reinterpretar el viejo `port_state.json` — así que el **próximo scan programado va a sembrar un baseline silencioso de nuevo** (una sola alerta resumen, no una avalancha) antes de que las alertas de "puerto nuevo" empiecen a bajar de verdad.
...
- `data/port_state.json` (el viejo, por IP) queda obsoleto — se puede borrar más adelante, no es urgente.
